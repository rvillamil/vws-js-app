    ...
    if (r == true) {
        ...
        return favoriteRepository
            ...
            .then(newShowCollection => {
                console.log(
                    `ShowCollection object saved!: ${JSON.stringify(newShowCollection)}\n`
                )
            })
            .catch(err => {
                console.error(err)
...
function deleteFavoriteTVshow(collectionName) {
    var r = confirm(
        `¿Quieres eliminar la serie '${collectionName}' de tus favoritos?!`
    )
    if (r == true) {
        return favoriteRepository
...
function onclickToDownloadFile(urltodownload) {
    ...
    favoriteRepository
        .updateAllreadyDownloadedShowByURL(urltodownload, true)
        .then(numReplaced => {
            console.log(
                `The episode with url '${urltodownload}' has been replaced '${numReplaced}' times`
            )
        })
}
...
function loadAndRenderFavoritesTVShowCollection(htmlElementID) {
    ...
    return crawlMyFavoritesTVShowCollection(
        CRAWL_TV_SHOWS_FAVORITES_LIMIT,
        htmlElementID
    )
        .then(showCollectionListCrawled => {
            ...
            return Promise.all(actions)
                .then(numReplaced => {
                    _refreshFromPersistence(htmlElementID).then(fullHTML => {
...
function _refreshFromPersistence(htmlElementID) {
    return favoriteRepository.findAll().then(docWithshowCollectionList => {
        var actions = docWithshowCollectionList.map(newDocWithShowCollection => {
            return renderShowCollectionBox(
